...
# path = '../qiime2_res/SOURCE_china_mets_table_norm/'
# name2 = 'China_all'; cols = c('#82CD47','#1F8A70','#3DB2FF','#EB5353')
# path = '../qiime2_res/SOURCE_china_mets_table_norm_rural/'
# name2 = 'China_rural'; cols = c('#7FB77E','#F7A76C')
# path = '../qiime2_res/SOURCE_china_mets_table_norm_urban_cd/'
# name2 = 'China_urban_cd'; cols = c('red','blue')
path = '../qiime2_res/SOURCE2022_Stool_Data_MZM13_N2_filtered_v2/'
name2 = 'Israel_dx'; cols = c('red','blue')


name = 'source'
# taxa_file = sprintf('%s/biom/%s_deblur_map_L7.1.txt',path, name)
taxa_file = sprintf('%s/map.tsv',path)
out_path =  sprintf('%s/R_res',path)
# pcoa_file = sprintf('%s/core-metrics-results/SOURCE_china_mets_table_t_norm_canberra_pcoa_result/ordination.txt',path)
pcoa_file = sprintf('%s/core-metrics-results/SOURCE2022_Stool_Data_MZM13_N2_filtered_v2_canberra_pcoa_results/ordination.txt',path)
...
# pcoa$Patient_group2[pcoa$Patient_group2=='Urban_health'] = 'Urban'
...
# pcoa$Patient_group2 = factor(pcoa$Patient_group2, levels = c('Rural','Rural-Urban','Urban','CD'))
library(ggside)
...
{
  g = g+ geom_xsidedensity(aes(fill = Dx, col = Dx, alpha = 0.5)) +
    geom_ysidedensity(aes(fill = Dx, col = Dx, alpha = 0.5)) +
    theme(ggside.panel.scale = 0.25)
} else if (side_type == 'boxplot')
{
  g = g + geom_xsideboxplot(orientation = "y", aes(col = Dx)) +
    geom_ysideboxplot(orientation = "x", aes(col = Dx)) +
    theme(ggside.panel.scale = 0.25)
}
